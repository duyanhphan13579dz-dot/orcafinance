...
function classify(change: number | null, forceNeutral: boolean) {
  if (change === null || !Number.isFinite(change)) return { status: "no-data" as const, color: "no-data" as const, intensity: 0 };
  if (forceNeutral) return { status: "unchanged" as const, color: "unchanged" as const, intensity: 0 };
  ...
  if (Math.abs(change) < 0.005) return { status: "unchanged" as const, color: "unchanged" as const, intensity: 0 };
  return {
...
async function warmFeaturedSnapshots() {
  try { await getQuotes(FEATURED_SYMBOLS); }
  catch (err) { logger.warn("heatmap_warm_failed", { error: err instanceof Error ? err.message : String(err) }); }
}

const globalUniverse = globalThis as typeof globalThis & { __orcaHeatmapUniverseSyncAt?: number; __orcaHeatmapUniverseSyncing?: boolean; __orcaHeatmapQuoteWarming?: boolean };
async function warmMissingQuoteBatch(symbols: string[]) {
  ...
  try {
    ...
    logger.info("heatmap_quote_batch_warmed", { requested: batch.length, received: quotes.length });
  } catch (err) {
    logger.warn("heatmap_quote_batch_failed", { error: err instanceof Error ? err.message : String(err) });
  } finally { globalUniverse.__orcaHeatmapQuoteWarming = false; }
}
  ...
  try {
    ...
    for (const response of responses) if (response.status === "fulfilled") {
      for (const row of response.value) {
        ...
        if (!/^[A-Z]{3}$/.test(row.symbol) || (!type.includes("CỔ PHIẾU") && !type.includes("STOCK"))) continue;
        const benchmark = getBenchmarkForSymbol(row.symbol);
        seen.set(row.symbol, { ...row, sector: benchmark.sector, industry: benchmark.industry });
      }
    }
    const rows = [...seen.values()];
    for (let i = 0; i < rows.length; i += 100) {
      await db.insert(companies).values(rows.slice(i, i + 100)).onConflictDoNothing({ target: companies.symbol });
    }
    globalUniverse.__orcaHeatmapUniverseSyncAt = Date.now();
    logger.info("heatmap_universe_synced", { symbols: rows.length });
  } catch (err) {
    logger.warn("heatmap_universe_sync_failed", { error: err instanceof Error ? err.message : String(err) });
  } finally { globalUniverse.__orcaHeatmapUniverseSyncing = false; }
}
...
export async function getMarketHeatmap(): Promise<{
  ...
  return cached("market:heatmap:v2", 5_000, async () => {
    void warmFeaturedSnapshots();
    void syncListedUniverse();
    const [companyRows, snapshots] = await Promise.all([
      db.select({ symbol: companies.symbol, name: companies.name, exchange: companies.exchange, sector: companies.sector, industry: companies.industry })
        .from(companies).orderBy(asc(companies.symbol)),
      db.select().from(priceSnapshots).orderBy(desc(priceSnapshots.updatedAt)),
    ]);

    const universe = new Map<string, { symbol: string; name: string; exchange: string; sector: string; industry: string }>();
    for (const symbol of FEATURED_SYMBOLS) {
      const benchmark = getBenchmarkForSymbol(symbol);
      universe.set(symbol, { symbol, name: symbol, exchange: "HOSE", sector: benchmark.sector, industry: benchmark.industry });
    }
    ...
    for (const snapshot of snapshots) if (!latest.has(snapshot.symbol)) latest.set(snapshot.symbol, snapshot);
    ...
    const items: HeatmapItem[] = [...universe.values()].map((company) => {
    ...
    for (const item of items) {
      const current = sectorMap.get(item.sector) ?? { name: item.sector, count: 0, tradingValue: 0 };
      current.count += 1; current.tradingValue += item.tradingValue;
      sectorMap.set(item.sector, current);
    }
    const stats = {
      ceiling: items.filter((x) => x.status === "ceiling").length,
      ...
      floor: items.filter((x) => x.status === "floor").length,
      "no-data": items.filter((x) => x.status === "no-data").length,
      total: items.length,
    };
    return {
      items, marketStatus, timestamp: new Date().toISOString(), stats,
      sectors: [...sectorMap.values()].sort((a, b) => b.tradingValue - a.tradingValue || b.count - a.count),
    };
  });
}
