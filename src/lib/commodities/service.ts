...
/** Calendar-day key at midnight in Asia/Ho_Chi_Minh, stored as UTC. */
function vietnamDayStart(input: Date): Date {
  ...
  // 00:00 ICT = 17:00 UTC of the previous calendar day.
  return new Date(Date.UTC(get("year"), get("month") - 1, get("day"), -7, 0, 0));
}

function convertToVnd(price: CommodityPriceData, rate: number): number {
  const unit = price.unit.toLowerCase();
  // USc / US cent means quoted in cents, not dollars.
  const monetaryValue = price.currency === "USD" && (unit.startsWith("usc") || unit.startsWith("us cent"))
    ? price.price / 100
    : price.price;
  ...
  for (const price of prices) {
    try {
      ...
      if (price.currency !== "VND") {
        ...
        if (rate) {
          priceVnd = convertToVnd(price, rate);
          currencyRate = rate;
        } else {
          log.warn("exchange_rate_not_found", { currency: price.currency, symbol: price.symbol });
          continue;
        }
      }
      const priceDate = vietnamDayStart(price.timestamp);
...
export async function refreshCommoditiesFromRealSources() {
  const { fetchAllCommoditiesData } = await import("./connectors");
  const startedAt = Date.now();
  await initializeCommodities();
  await initializeStockImpacts();
  // Remove snapshots created by the legacy demo connector. The real module
  // intentionally accepts commodity values only from these two audited pages.
  const realSources = ["simplize.vn/hang-hoa", "data.vietnambiz.vn/goods"];
  const deletedLegacy = await db.delete(commodityPrices)
    .where(notInArray(commodityPrices.source, realSources))
    .returning({ id: commodityPrices.id });
  if (deletedLegacy.length) log.warn("legacy_demo_prices_removed", { count: deletedLegacy.length });
  const fetched = await fetchAllCommoditiesData();
  const ratesSaved = fetched.exchangeRates.length ? await saveExchangeRates(fetched.exchangeRates) : 0;
  const pricesSaved = await saveCommodityPrices(fetched.prices);
  return {
    pricesFetched: fetched.prices.length,
    pricesSaved,
    legacyRowsRemoved: deletedLegacy.length,
    ratesSaved,
...
export async function ensureCommoditiesFresh(maxAgeMs = 15 * 60_000) {
  ...
  const refreshed = await refreshCommoditiesFromRealSources();
