...
export async function generateMorningBrief(date: Date = new Date()): Promise<{ id?: number; html: string; type: ReportType; date: string }> {
  ...
  if (id) void import("@/lib/notifications/service").then(({ dispatchToEligibleUsers }) =>
    dispatchToEligibleUsers({ category: "morning_report", title: `Morning Brief ${dateKey}`, body: "Báo cáo đầu ngày mới đã sẵn sàng.", url: `/reports?type=morning&date=${dateKey}` }),
  ).catch((err) => log.warn("notification_hook_failed", { type: "morning", error: String(err) }));
...
export async function generateMarketSummary(date: Date = new Date()): Promise<{ id?: number; html: string; type: ReportType; date: string }> {
  ...
  if (id) void import("@/lib/notifications/service").then(({ dispatchToEligibleUsers }) =>
    dispatchToEligibleUsers({ category: "summary_report", title: `Market Summary ${dateKey}`, body: "Nhận định cuối phiên và chiến lược phiên tới đã sẵn sàng.", url: `/reports?type=summary&date=${dateKey}` }),
  ).catch((err) => log.warn("notification_hook_failed", { type: "summary", error: String(err) }));
