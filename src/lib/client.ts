...
export async function api<T>(path: string, init?: RequestInit): Promise<Envelope<T>> {
  ...
  if (!contentType.includes("application/json")) {
    ...
    throw new Error(`API trả về ${contentType || "dữ liệu không xác định"} (HTTP ${res.status})${snippet ? `: ${snippet}` : ""}`);
  }
  let json: Envelope<T>;
  try {
    json = JSON.parse(text) as Envelope<T>;
  } catch {
    throw new Error(`API trả về JSON không hợp lệ (HTTP ${res.status})`);
  }
