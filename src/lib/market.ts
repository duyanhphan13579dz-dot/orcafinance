...
export async function getQuote(symbol: string): Promise<Quote> {
  ...
  // Capture the previous real snapshot before overwriting it. This enables
  // exact short-window movement detection (>5% within 5 minutes) without
  // changing the quote-fetching business logic.
  const previousSnapshot = await db.select({ close: priceSnapshots.close, updatedAt: priceSnapshots.updatedAt })
    .from(priceSnapshots).where(eq(priceSnapshots.symbol, symbol)).limit(1).catch(() => []);
  ...
  const prior = previousSnapshot[0];
  const withinFiveMinutes = prior && Date.now() - prior.updatedAt.getTime() <= 5 * 60_000;
  const shortMovePct = prior?.close > 0 ? ((quote.close - prior.close) / prior.close) * 100 : null;
  if (withinFiveMinutes && shortMovePct !== null && Math.abs(shortMovePct) >= 5) {
    void import("@/lib/notifications/service").then(({ dispatchToEligibleUsers }) =>
      dispatchToEligibleUsers({
        category: "price_alert",
        title: `${quote.symbol} biến động ${shortMovePct >= 0 ? "+" : ""}${shortMovePct.toFixed(2)}% trong 5 phút`,
        body: `Giá hiện tại ${quote.close.toLocaleString("vi-VN")} · Nguồn ${quote.source}`,
        url: `/stocks/${quote.symbol}`,
      }),
    ).catch((err) => logger.warn("price_notification_hook_failed", { symbol: quote.symbol, error: String(err) }));
  }
  return quote;
}
        ...
        if (Math.abs(sentimentScore) >= 0.4) {
          void import("@/lib/notifications/service").then(({ dispatchToEligibleUsers }) =>
            dispatchToEligibleUsers({
              ...
              title: item.title,
              body: `${item.sourceName} · Sentiment ${sentimentScore.toFixed(2)}`,
              url: item.link,
            }),
          ).catch((err) => logger.warn("news_notification_hook_failed", { error: String(err) }));
        }
      }
    } catch (err) {
