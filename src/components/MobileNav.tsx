...
const NAV = [
  ...
  { href: "/system", label: "Hệ thống", icon: "⚙️" },
];
        ...
        {open && (
          <div className="absolute top-full left-0 right-0 bg-[#0A2540] border-b border-[#1a3558] shadow-2xl animate-in slide-in-from-top-2">
            <nav className="p-2">
              ...
              <div className="border-t border-[#1a3558] mt-2 pt-2">
                {user ? <>
                  <Link href="/settings" onClick={()=>setOpen(false)} className="flex min-h-11 items-center gap-3 rounded-lg px-4 py-3 text-slate-300"><span>⚙️</span>Cài đặt</Link>
                  <button onClick={()=>void logout()} className="w-full min-h-11 text-left rounded-lg px-4 py-3 text-rose-300">Đăng xuất</button>
                </> : <div className="grid grid-cols-2 gap-2 p-2"><Link href="/auth/login" onClick={()=>setOpen(false)} className="btn-orca-outline text-center py-3">Đăng nhập</Link><Link href="/auth/register" onClick={()=>setOpen(false)} className="btn-orca text-center py-3">Đăng ký</Link></div>}
              </div>
            </nav>
          </div>
        )}
