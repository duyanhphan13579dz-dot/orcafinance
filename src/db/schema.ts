...
// ═══════════════════════════════════════════════════════════════════════
// Commodities Module — re-export from lib/commodities/schema
// ═══════════════════════════════════════════════════════════════════════
export {
  impactTypeEnum,
  commodityGroupEnum,
...
export const userPreferences = pgTable("user_preferences", {
  ...
  fontFamily: varchar("font_family", { length: 40 }).notNull().default("inter"),
  ...
  inAppNews: boolean("in_app_news").notNull().default(true),
  inAppAi: boolean("in_app_ai").notNull().default(true),
  updatedAt: timestamp("updated_at", { withTimezone: true }).defaultNow().notNull(),
});
...
}, (t) => [index("user_sessions_user_idx").on(t.userId), index("user_sessions_token_idx").on(t.token)]);
...
export const notificationPreferences = pgTable("notification_preferences", {
  ...
  emailMorning: boolean("email_morning").notNull().default(true),
  emailSummary: boolean("email_summary").notNull().default(true),
  emailPriceAlerts: boolean("email_price_alerts").notNull().default(false),
  emailBreakingNews: boolean("email_breaking_news").notNull().default(false),
  pushPriceAlerts: boolean("push_price_alerts").notNull().default(false),
  pushBreakingNews: boolean("push_breaking_news").notNull().default(false),
  pushReports: boolean("push_reports").notNull().default(true),
  updatedAt: timestamp("updated_at", { withTimezone: true }).defaultNow().notNull(),
});
...
export const notificationLogs = pgTable("notification_logs", {
  id: uuid("id").primaryKey().defaultRandom(),
  userId: uuid("user_id").notNull().references(() => users.id, { onDelete: "cascade" }),
  type: varchar("type", { length: 20 }).notNull(), // push | email | in_app
  ...
  error: text("error"),
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
}, (t) => [index("notification_logs_user_idx").on(t.userId), index("notification_logs_created_idx").on(t.createdAt)]);
